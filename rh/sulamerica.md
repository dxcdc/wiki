---
title: Seguro de Vida (SulAmérica)
description: Consulta de apólices e informações de cobertura de seguro de vida empresarial
published: true
date: 2026-07-14T23:59:00.000Z
tags: 'rh, sulamerica'
editor: markdown
---

# 👥 Seguro de Vida (SulAmérica)

Consulta de apólices e informações de cobertura de seguro de vida empresarial.

---

## 🚀 Link de Acesso
👉 **[Clique aqui para Acessar](https://vidaweb.sulamerica.com.br/)**

---

## 📚 Materiais de Aprendizado (Apostilas)

* 📕 **[Apostila: Treinamento Inicial de Seguro de Vida (SulAmérica)]** — Guia básico de integração e primeiros passos.
* 📖 **[Guia Rápido: Boas Práticas]** — Recomendações importantes sobre segurança e usabilidade.

---

## 📖 Tutoriais e Procedimentos

* 📝 **[Como abrir chamados para o Seguro de Vida (SulAmérica)]** — Fluxo para reportar problemas de acesso ou bugs nesta ferramenta.
* *(Espaço reservado para procedimentos específicos da organização - clique em Editar para alimentar).*

---

## 🔄 Atualizações e Avisos

* **Julho de 2026:** Página do portal de sistemas ativada na Wiki do CDC.
