---
title: Cadastro de Sistemas
description: Solicitação para criação de novos usuários e alteração de níveis de permissão em sistemas do CDC
published: true
date: 2026-07-14T23:59:00.000Z
tags: 'ti, cadastro-sistemas'
editor: markdown
---

# 🛠️ Cadastro de Sistemas

Solicitação para criação de novos usuários e alteração de níveis de permissão em sistemas do CDC.

---

## 🚀 [Acessar o Cadastro de Sistemas](https://docs.google.com/forms/d/e/1FAIpQLSfb0De_QY09xVPunhCEzpTZeV965EQt_Cmk4vL-WwgqzoaOpA/viewform?ouid=103765690399295332532&usp=sharing)

---

## 📚 Materiais de Aprendizado (Apostilas)

* 📕 **[Apostila: Treinamento Inicial de Cadastro de Sistemas]** — Guia básico de integração e primeiros passos.
* 📖 **[Guia Rápido: Boas Práticas]** — Recomendações importantes sobre segurança e usabilidade.

---

## 📖 Tutoriais e Procedimentos

* 📝 **[Como abrir chamados para o Cadastro de Sistemas]** — Fluxo para reportar problemas de acesso ou bugs nesta ferramenta.
* *(Espaço reservado para procedimentos específicos da organização - clique em Editar para alimentar).*

---

## 🔄 Atualizações e Avisos

* **Julho de 2026:** Página do portal de sistemas ativada na Wiki do CDC.
