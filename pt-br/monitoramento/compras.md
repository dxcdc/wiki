---
title: Compras ONGSYS
description: Painel de acompanhamento de requisições de compras e cotações de fornecedores
published: true
date: 2026-07-14T23:59:00.000Z
tags: 'monitoramento, compras'
editor: markdown
---

# 📊 Compras ONGSYS

Painel de acompanhamento de requisições de compras e cotações de fornecedores.

---

## 🚀 [Acessar o Compras ONGSYS](https://erpcompras.cdc.org.br/)

---

## 📚 Materiais de Aprendizado (Apostilas)

* 📕 **[Apostila: Treinamento Inicial de Compras ONGSYS]** — Guia básico de integração e primeiros passos.
* 📖 **[Guia Rápido: Boas Práticas]** — Recomendações importantes sobre segurança e usabilidade.

---

## 📖 Tutoriais e Procedimentos

* 📝 **[Como abrir chamados para o Compras ONGSYS]** — Fluxo para reportar problemas de acesso ou bugs nesta ferramenta.
* *(Espaço reservado para procedimentos específicos da organização - clique em Editar para alimentar).*

---

## 🔄 Atualizações e Avisos

* **Julho de 2026:** Página do portal de sistemas ativada na Wiki do CDC.
